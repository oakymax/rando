...
class photo {
    function actionGet(){
        ...
        switch ($filter) {
            case 'incoming':
                $rows = SimplePDO::getInstance()->get_results("SELECT storage_identifier FROM photo WHERE recipient = ? ORDER BY sent_time DESC", [Auth::userid()]);
                break;

            case 'outgoing':
                $rows = SimplePDO::getInstance()->get_results("SELECT storage_identifier FROM photo WHERE sender = ? ORDER BY sent_time DESC", [Auth::userid()]);
                break;

            case 'all':
                $rows = SimplePDO::getInstance()->get_results("SELECT storage_identifier FROM photo WHERE sender = ? or recipient = ? ORDER BY sent_time DESC", [Auth::userid(), Auth::userid()]);
                break;
        }
